...
from explain_module.utils import set_seed, Gtrain, Gtest
...
def parse_args():
    ...
    parser.add_argument('--data_path', nargs='?', default=osp.join(osp.dirname(__file__), '..', 'data', 'BA2'),
                        help='Input data path.')
    parser.add_argument('--model_path', nargs='?', default=osp.join(osp.dirname(__file__), '..',  'trained/gnns/'),
...
class BA2MotifNet(torch.nn.Module):
    def __init__(self, num_unit):
        super().__init__()

        self.num_unit = num_unit

        self.node_emb = Linear(10, 64)
...
if __name__ == '__main__':
    ...
    model = BA2MotifNet(args.num_unit).to(device)
    ...
    if args.random_label:
        save_path = 'ba2_net_rd.pt'
    else:
        save_path = 'ba2_net.pt'
    torch.save(model, osp.join('E:/USTC/CODE_ZERO/Causal-Screening-master-fjf/explain_module', save_path))
