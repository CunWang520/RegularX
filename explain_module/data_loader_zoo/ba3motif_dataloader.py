...
class BA3Motif(InMemoryDataset):
    ...
    def process(self):
        ...
        edge_index_list, label_list, ground_truth_list, role_id_list, pos = \
            np.load(osp.join(self.raw_dir, self.raw_file_names[0]), allow_pickle=True)

        data_list = []
        for idx, (edge_index, y, ground_truth, z, p) in \
                enumerate(zip(edge_index_list, label_list, ground_truth_list, role_id_list, pos)):
...
#Epoch  19[2.293s]: LR: 0.00100, Loss: 0.02335, Train acc: 0.99364, Validation Loss: 0.01105, Validation acc: 0.997500
#Epoch  20[2.572s]: LR: 0.00100, Loss: 0.02266, Test Loss: 0.01855, Test acc: 0.99250
