...
class ReFine(Explainer):
    ...
    def __init__(self, device, gnn_model,
                 ...
                 e_in_channels=3,
                 hid=50, n_layers=2,
                 n_label=2, gamma=1, reg_enlarge=1,
                 ):
    ...
    def fidelity_loss(self, log_logits, mask, pred_label):
        ...
        loss = loss + self.coeffs['edge_size'] * mask.mean() * (self.reg_enlarge * 1000)
        ent = -mask * torch.log(mask + EPS) - (1 - mask) * torch.log(1 - mask + EPS)
        loss = loss + self.coeffs['edge_ent'] * ent.mean() * (self.reg_enlarge * 1000)
        return loss
