...
def parse_args():
    ...
    parser.add_argument('--reg_enlarge', type=float, default=1)
...
logger = Logger.init_logger(filename=log_root + f"/refine-{args.dataset}-{args.reg_enlarge}.log" )
...
explainer = ReFine(device, path, gamma=args.gamma, reg_enlarge=args.reg_enlarge,
...
torch.save(explainer, osp.join(model_dir, f'{args.dataset}-{args.reg_enlarge}.pt'))
